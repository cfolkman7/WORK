...
#opens serial port 
ser = serial.Serial('COM9', 9600, stopbits = 1, parity='N', bytesize=8, timeout=.5)
...
#closes serial port at end of program
def clean_exit():
    ser.close()
...
def execute_program():
    ...
    def measure():
        def measurement():
            ser.write(bytes('D\n', 'ASCII'))
            lasermik_data = str(ser.read_until())
            lasermik_data = lasermik_data[1:7].decode('utf-8')
