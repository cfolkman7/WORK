...
#boolean value used to check completed work orders
completed_WO = False
...
df_2 = pd.read_excel(io=read_file, sheet_name=sheet_name_hub)
...
for filepath in glob.iglob('//USSJFS1/SJ_PUBLIC/MAESTRO/MAESTRO/TENSILE TESTER DATA FILES/HUB TENSILE/*.xlsx'):
    ...
    while i < len(completed_WO_list):
        if filepath[74:82] == completed_WO_list[i]:
            completed_WO = True
            break
...
for filepath in glob.iglob('//USSJFS1/SJ_PUBLIC/MAESTRO/MAESTRO/TENSILE TESTER DATA FILES/TIP TENSILE/*.xlsx'):
    df_2 = pd.read_excel(io=read_file, sheet_name=sheet_name_tip)
    completed_WO_list = df_2['WO:'].to_list()
    i = 0
    while i < len(completed_WO_list):
        if filepath[74:82] == completed_WO_list[i]:
...
for filepath in glob.glob('//USSJFS1/SJ_PUBLIC/MAESTRO/MAESTRO/TENSILE TESTER DATA FILES/MB TENSILE/*.xlsx'):
    df_2 = pd.read_excel(io=read_file, sheet_name=sheet_name_mb)
    completed_WO_list = df_2['WO:'].to_list()
    i = 0
    while i < len(completed_WO_list):
        if filepath[73:81] == completed_WO_list[i]:
            completed_WO = True
            break
        else: completed_WO = False
        i += 1
